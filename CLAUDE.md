...
v0.1 구현 완료. push/pull/list 동작 확인. skill/command 래핑 미착수.
...
## Session File Structure (검증 완료)

- `~/.claude/projects/<프로젝트>/` — 프로젝트별 대화 기록 (UUID.jsonl + UUID/subagents/)
- `~/.claude/sessions/{pid}.json` — 세션 인덱스 (claude --resume에 필요)
- `~/.claude/transcripts/` — resume과 무관, ccrelay scope 밖
